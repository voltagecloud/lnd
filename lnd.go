...
func Main(cfg *Config, lisCfg ListenerCfg, implCfg *ImplementationCfg,
	...
	if cfg.TLSEncryptKey {
		...
		_, _, _, _, _, err = getTLSConfig(cfg, activeChainControl.KeyRing)
		if err != nil {
...
func getTLSConfig(cfg *Config, keyRing keychain.KeyRing) (
	...
	func(net.Addr) (net.Listener, error), func(), *cert.TlsReloader, error) {

	// If TLS Key Encryption is on but the keyring is empty then this
	// is a temporary certificate.
	var emptyKeyRing keychain.KeyRing
	var certData tls.Certificate
	var parsedCert *x509.Certificate
	var certBytes []byte
	var keyBytes []byte
	var err error
	var certPath string
	if cfg.TLSEncryptKey && (keyRing == emptyKeyRing) {
		...
		certBytes, keyBytes, err = cert.GenCertPair(
			...
			cfg.TLSDisableAutofill, tmpValidity,
		)
		if err != nil {
		...
		if !fileExists(cfg.TLSCertPath) && !fileExists(cfg.TLSKeyPath) {
			...
			if cfg.TLSEncryptKey {
				...
				if err = ioutil.WriteFile(cfg.TLSKeyPath, b.Bytes(),
					0600); err != nil {
					return nil, nil, nil, nil, nil, err
				}
			} else {
				keyBuf := bytes.NewBuffer(keyBytes)
				if err = ioutil.WriteFile(cfg.TLSKeyPath, keyBuf.Bytes(),
					0600); err != nil {
					return nil, nil, nil, nil, nil, err
				}
			}
			...
			rpcsLog.Info("Gettings the existing cert data")
			certBytes, keyBytes, err = cert.GetCertBytesFromPath(cfg.TLSCertPath, cfg.TLSKeyPath)
			if err != nil {
				return nil, nil, nil, nil, nil, err
			}
	...
	return serverOpts, restDialOpts, restListen, cleanUp, tlsr, nil
}
