...
func (t *TLSManager) getTLSKey() error {
	...
	if !bytes.HasPrefix(t.keyBytes, privateKeyPrefix) {
		...
		encryptKeyBytes, err := lnencrypt.GenEncryptionKey(t.keyRing)
		if err != nil {
			return err
		}
		keyBytes, err := lnencrypt.Encrypter{}.DecryptPayloadFromReader(
			reader, encryptKeyBytes,
		)
		if err != nil {
			return err
		}
		...
		err = lnencrypt.Encrypter{}.EncryptPayloadToWriter(*keyBuf, &b, encryptKeyBytes)
		if err != nil {
			return err
		}
...
// option is set.
func (t *TLSManager) setUpLetsEncrypt(tlsCfg *tls.Config) func() {
...
func (t *TLSManager) reloadCertificate() error {
	...
	keyBytes, err := lnencrypt.Encrypter{}.DecryptPayloadFromReader(reader, encryptKeyBytes)
	if err != nil {
		return err
	}
	t.keyBytes = keyBytes
