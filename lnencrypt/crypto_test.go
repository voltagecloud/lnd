...
func TestEncryptDecryptPayload(t *testing.T) {
	...
	for i, payloadCase := range payloadCases {
		...
		encryptKey, err := GenEncryptionKey(keyRing)
		if err != nil {
		...
		err = encrypter.EncryptPayloadToWriter(
			*payloadReader, &cipherBuffer, encryptKey,
		)
		if err != nil {
		...
		plaintext, err := encrypter.DecryptPayloadFromReader(&cipherBuffer, encryptKey)
...
func TestInvalidKeyGeneration(t *testing.T) {
	...
	_, err := GenEncryptionKey(&MockKeyRing{true})
	if err == nil {
