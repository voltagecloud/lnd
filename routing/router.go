		...
		case chainUpdate, ok := <-r.newBlocks:
			...
			currentHeight := atomic.LoadUint32(&r.bestHeight)
				...
				if chainUpdate.Height > currentHeight+1 {
					...
					if epochClient == nil {
						epochClient, err := r.cfg.Notifier.RegisterBlockEpochNtfn(outdatedBlock)
						if err != nil {
							log.Errorf("unable to register for epoch notifications")
						}
						r.cfg.BlockNotifier = *epochClient
					}
