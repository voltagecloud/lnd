...
func TestTLSAutoRegeneration(t *testing.T) {
	...
	removeFiles, certDir, certPath, keyPath, expiredCert, _ := writeTestCertFiles(
		t, true, false, nil,
	)
	defer removeFiles(certDir)
...
func TestTLSEncryptSetWhileKeyFileIsPlaintext(t *testing.T) {
	...
	removeFiles, certDir, certPath, keyPath, _,
		_ := writeTestCertFiles(
...
func writeTestCertFiles(t *testing.T, expiredCert, encryptTLSKey bool,
	...
	*x509.Certificate, []byte) {
	...
	// Either create a valid certificate or an expired certificiate pair,
	...
	return removeFiles, tempDirPath, certPath, keyPath, parsedCert, keyBuf.Bytes()
}
