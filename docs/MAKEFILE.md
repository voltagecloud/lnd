...
`sample-conf-check`
-------------------
Checks whether all required options of `lnd --help` are included in [sample-lnd.conf](github.com/voltagecloud/lnd/blob/master/sample-lnd.conf) and that the default values of `lnd --help` are also mentioned correctly.
