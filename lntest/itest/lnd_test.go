...
func testAddPeerConfig(net *lntest.NetworkHarness, t *harnessTest) {
	...
	parsedPeerAddr, err := lncfg.ParseLNAddressString(alicePeerAddress, strconv.Itoa(9735), network.ResolveTCPAddr)
        if err != nil {
                t.Fatalf("Couldn't parse address string: %v", err) 
        }

	parsedKeyStr := fmt.Sprintf("%x", parsedPeerAddr.IdentityKey.SerializeCompressed())
